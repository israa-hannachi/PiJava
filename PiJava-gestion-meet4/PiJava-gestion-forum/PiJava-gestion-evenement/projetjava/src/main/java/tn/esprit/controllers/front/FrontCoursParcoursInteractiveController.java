...
public class FrontCoursParcoursInteractiveController implements Initializable {
    ...
    private VBox createCourseCard(Cours cours) {
        ...
        card.setStyle("-fx-background-color:#f8fafc; -fx-background-radius:10; -fx-padding:14; -fx-border-color:#e2e8f0; -fx-border-radius:10; -fx-cursor:hand;");

        // ── En-tête : Ordre + Titre + Drag handle ──
        HBox header = new HBox(8);
        header.setAlignment(javafx.geometry.Pos.CENTER_LEFT);

        Label orderLabel = new Label("#" + cours.getOrdre());
        orderLabel.setStyle("-fx-background-color:#0FB5A9; -fx-text-fill:white; -fx-background-radius:6; -fx-font-weight:700; -fx-font-size:11; -fx-padding:2 8;");

        Label titleLabel = new Label(cours.getTitre());
        titleLabel.setStyle("-fx-font-size:14; -fx-font-weight:700; -fx-text-fill:#1f2937;");
        titleLabel.setWrapText(true);
        HBox.setHgrow(titleLabel, Priority.ALWAYS);

        Label dragHandle = new Label("⠿");
        dragHandle.setStyle("-fx-text-fill:#94a3b8; -fx-font-size:18; -fx-cursor:move;");
        dragHandle.setTooltip(new Tooltip("Glisser pour réorganiser"));

        header.getChildren().addAll(orderLabel, titleLabel, dragHandle);
        ...
        infoRow.getChildren().add(dureeLabel);

        boolean hasPdf = cours.getFichierContenu() != null && !cours.getFichierContenu().isEmpty();
        if (hasPdf) {
            Label pdfBadge = new Label("📄 PDF");
            pdfBadge.setStyle("-fx-background-color:#eff6ff; -fx-text-fill:#2563eb; -fx-background-radius:6; -fx-padding:2 8; -fx-font-size:11;");
            infoRow.getChildren().add(pdfBadge);
        }

        if (cours.getResumeAi() != null && !cours.getResumeAi().isEmpty()) {
            Label aiBadge = new Label("✨ IA");
            aiBadge.setStyle("-fx-background-color:#f5f3ff; -fx-text-fill:#7c3aed; -fx-background-radius:6; -fx-padding:2 8; -fx-font-size:11; -fx-font-weight:bold;");
            infoRow.getChildren().add(aiBadge);
        }
        ...
        statusLabel.setStyle("-fx-text-fill:#374151; -fx-font-size:12; -fx-font-weight:600;");
        ...
        // ── Notes (textarea) ──
        VBox notesSection = new VBox(4);
        notesSection.setStyle("-fx-background-color:white; -fx-background-radius:6; -fx-padding:8; -fx-border-color:#f1f5f9; -fx-border-radius:6;");
        Label notesTitle = new Label("📝 Notes personnelles");
        notesTitle.setStyle("-fx-font-size:11; -fx-font-weight:700; -fx-text-fill:#374151;");
        TextArea notesArea = new TextArea();
        notesArea.setPromptText("Ajoutez vos notes ici...");
        notesArea.setStyle("-fx-background-color:white; -fx-border-color:#e2e8f0; -fx-border-radius:4; -fx-padding:6; -fx-font-size:11;");
        notesArea.setPrefRowCount(2);

        // Charger notes existantes
        String courseNotes = loadCourseNotes(cours.getId());
        notesArea.setText(courseNotes != null ? courseNotes : "");
        notesArea.textProperty().addListener((obs, oldVal, newVal) -> saveCourseNotes(cours.getId(), newVal));
        notesSection.getChildren().addAll(notesTitle, notesArea);
        ...
        if (hasPdf) {
            ...
            viewPdfBtn.setStyle("-fx-background-color:#eff6ff; -fx-text-fill:#2563eb; -fx-background-radius:6; -fx-padding:4 10; -fx-font-size:11; -fx-cursor:hand;");
            viewPdfBtn.setOnAction(e -> {
                try {
                    if (cours.getFichierContenu().startsWith("http")) {
                        java.awt.Desktop.getDesktop().browse(new java.net.URI(cours.getFichierContenu()));
                    } else {
                        java.io.File f = new java.io.File(cours.getFichierContenu());
                        if (f.exists()) java.awt.Desktop.getDesktop().open(f);
                    }
                } catch (Exception ex) { ex.printStackTrace(); }
            });
            actionsRow.getChildren().add(viewPdfBtn);
        }

        if (cours.getContenu() != null && !cours.getContenu().isEmpty()) {
            Button viewContentBtn = new Button("📖 Lire le contenu");
            viewContentBtn.setStyle("-fx-background-color:#f0fdf4; -fx-text-fill:#166534; -fx-background-radius:6; -fx-padding:4 10; -fx-font-size:11; -fx-cursor:hand;");
            viewContentBtn.setOnAction(e -> showCourseContentDialog(cours));
            actionsRow.getChildren().add(viewContentBtn);
        }

        // Assembler la carte
        card.getChildren().addAll(header, infoRow);
        if (!descBox.getChildren().isEmpty()) card.getChildren().add(descBox);
        card.getChildren().addAll(statusRow, notesSection);
        if (!actionsRow.getChildren().isEmpty()) card.getChildren().add(actionsRow);

        // Hover effect
        card.setOnMouseEntered(e -> card.setStyle("-fx-background-color:#f0fffe; -fx-background-radius:10; -fx-padding:14; -fx-border-color:#0FB5A9; -fx-border-radius:10; -fx-cursor:hand; -fx-effect:dropshadow(three-pass-box,rgba(15,181,169,0.15),8,0,0,2);"));
        card.setOnMouseExited(e -> card.setStyle("-fx-background-color:#f8fafc; -fx-background-radius:10; -fx-padding:14; -fx-border-color:#e2e8f0; -fx-border-radius:10; -fx-cursor:hand;"));

        return card;
    }

    // Dialog pour afficher le contenu du cours
    private void showCourseContentDialog(Cours cours) {
        Dialog<Void> dlg = new Dialog<>();
        dlg.setTitle("📖 " + cours.getTitre());
        ...
        title.setStyle("-fx-font-size:20; -fx-font-weight:800; -fx-text-fill:#1f2937;");

        Label info = new Label("⏱ " + cours.getDuree() + " min  |  Ordre #" + cours.getOrdre());
        info.setStyle("-fx-text-fill:#64748b; -fx-font-size:12;");

        javafx.scene.web.WebView wv = new javafx.scene.web.WebView();
        wv.setPrefHeight(350);
        String html = "<html><body style='font-family:Segoe UI,sans-serif; line-height:1.6; color:#374151; padding:10px;'>" +
            (cours.getContenu() != null ? cours.getContenu() : "<p style='color:#9ca3af;'>Aucun contenu.</p>") +
            "</body></html>";
        wv.getEngine().loadContent(html);

        root.getChildren().addAll(title, info, wv);

        // Résumé IA si disponible
        if (cours.getResumeAi() != null && !cours.getResumeAi().isEmpty()) {
            VBox aiBox = new VBox(6);
            aiBox.setStyle("-fx-background-color:#f5f3ff; -fx-padding:12; -fx-background-radius:10; -fx-border-color:#ddd6fe;");
            Label aiTitle = new Label("✨ Résumé IA");
            aiTitle.setStyle("-fx-font-weight:800; -fx-text-fill:#6d28d9; -fx-font-size:13;");
            Label aiText = new Label(cours.getResumeAi());
            aiText.setWrapText(true);
            aiText.setStyle("-fx-text-fill:#4c1d95; -fx-font-size:12;");
            aiBox.getChildren().addAll(aiTitle, aiText);
    ...
    private VBox createRoadmapCard(Cours_Module module) {
        ...
        int pct = moduleCourses.isEmpty() ? 0 : (int)((completedCount * 100) / moduleCourses.size());
        String progressText = completedCount + "/" + moduleCourses.size() + " cours terminés (" + pct + "%)";

        Label progressLabel = new Label(progressText);
        progressLabel.setStyle("-fx-text-fill:#64748b; -fx-font-size:13;");
        ...
        // Liste des cours avec statut + détails
        VBox coursListBox = new VBox(6);
        for (Cours cours : moduleCourses) {
            String st = courseStatus.getOrDefault(cours.getId(), "À faire");
            String icon = "Terminé".equals(st) ? "✅" : "En cours".equals(st) ? "🔄" : "⬜";

            HBox courseRow = new HBox(8);
            courseRow.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
            courseRow.setStyle("-fx-background-color:#f8fafc; -fx-background-radius:6; -fx-padding:6 10;");

            Label statusIcon = new Label(icon);
            statusIcon.setStyle("-fx-font-size:14;");

            Label courseTitle = new Label(cours.getTitre());
            courseTitle.setStyle("-fx-text-fill:#1f2937; -fx-font-size:12; -fx-font-weight:600;");
            HBox.setHgrow(courseTitle, Priority.ALWAYS);

            Label durationLabel = new Label("⏱ " + cours.getDuree() + "min");
            durationLabel.setStyle("-fx-text-fill:#94a3b8; -fx-font-size:11;");

            Label statusBadge = new Label(st);
            String badgeColor = "Terminé".equals(st) ? "-fx-background-color:#dcfce7; -fx-text-fill:#166534;" :
                "En cours".equals(st) ? "-fx-background-color:#fef3c7; -fx-text-fill:#92400e;" :
                "-fx-background-color:#f3f4f6; -fx-text-fill:#6b7280;";
            statusBadge.setStyle(badgeColor + " -fx-background-radius:6; -fx-padding:2 8; -fx-font-size:10; -fx-font-weight:700;");

            courseRow.getChildren().addAll(statusIcon, courseTitle, durationLabel, statusBadge);
            coursListBox.getChildren().add(courseRow);
        }
    ...
    private void initializeNewStructure() {
        ...
        // Récupérer les IDs de modules réellement présents dans coursList
        Set<Integer> actualModuleIds = coursList.stream()
            .map(Cours::getModuleId)
            .collect(Collectors.toCollection(LinkedHashSet::new));

        if (moduleOrder.isEmpty()) {
            moduleOrder.addAll(actualModuleIds);
        } else {
            // Nettoyer l'ordre sauvegardé : retirer les modules sans cours
            moduleOrder.removeIf(id -> !actualModuleIds.contains(id));
            // Ajouter les nouveaux modules non encore dans l'ordre
            for (Integer id : actualModuleIds) {
        ...
        // Toujours recalculer pour éviter les données obsolètes
        ...
        for (Map.Entry<Integer, List<Cours>> e : grouped.entrySet()) {
            List<Integer> savedOrder = courseOrderByModule.get(e.getKey());
            List<Integer> actualIds = e.getValue().stream()
                .sorted(Comparator.comparingInt(Cours::getOrdre))
                .map(Cours::getId)
                .collect(Collectors.toList());

            if (savedOrder == null || savedOrder.isEmpty()) {
                courseOrderByModule.put(e.getKey(), new ArrayList<>(actualIds));
            } else {
                // Nettoyer l'ordre sauvegardé
                savedOrder.removeIf(id -> !actualIds.contains(id));
                // Ajouter les nouveaux cours
                for (Integer id : actualIds) {
                    if (!savedOrder.contains(id)) savedOrder.add(id);
                }
            }
        }
        // Retirer les modules sans cours dans courseOrderByModule
        courseOrderByModule.keySet().removeIf(id -> !actualModuleIds.contains(id));
