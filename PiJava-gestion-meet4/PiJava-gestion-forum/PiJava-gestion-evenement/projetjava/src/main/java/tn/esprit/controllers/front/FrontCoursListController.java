...
public class FrontCoursListController implements Initializable {
    ...
    private VBox buildCoursCard(Cours c, boolean isFavTab) {
        ...
        card.setStyle("-fx-background-color:linear-gradient(to bottom right,#ffffff,#f7fffd); -fx-background-radius:22; -fx-padding:22; -fx-border-color:rgba(20,184,166,0.16); -fx-border-radius:22; -fx-effect:dropshadow(gaussian,rgba(15,23,42,0.10),22,0,0,8);");

        // ── Ligne haut : poignée + ordre + durée + ⭐ ──────────────────────────────────
        HBox topRow = new HBox(8);
        topRow.setAlignment(javafx.geometry.Pos.CENTER_LEFT);

        if (isProf) {
            Label dragHandle = new Label("↕");
            dragHandle.setStyle("-fx-text-fill:#94a3b8; -fx-font-size:18; -fx-font-weight:bold; -fx-cursor:move;");
            dragHandle.setTooltip(new Tooltip("Maintenez pour réorganiser"));
            
            // On peut aussi ajouter des boutons Monter/Descendre pour une réorganisation plus précise
            Button upBtn = new Button("🔼");
            upBtn.setStyle("-fx-background-color:transparent; -fx-padding:0; -fx-cursor:hand;");
            upBtn.setOnAction(e -> handleMoveUp(c));
            
            Button downBtn = new Button("🔽");
            downBtn.setStyle("-fx-background-color:transparent; -fx-padding:0; -fx-cursor:hand;");
            downBtn.setOnAction(e -> handleMoveDown(c));
            
            topRow.getChildren().addAll(dragHandle, upBtn, downBtn);
        }
        ...
        // Hover
        card.setOnMouseEntered(e -> card.setStyle("-fx-background-color:linear-gradient(to bottom right,#ffffff,#ecfeff); -fx-background-radius:22; -fx-padding:22; -fx-border-color:#14b8a6; -fx-border-radius:22; -fx-effect:dropshadow(gaussian,rgba(20,184,166,0.26),28,0,0,10);"));
        card.setOnMouseExited(e -> card.setStyle("-fx-background-color:linear-gradient(to bottom right,#ffffff,#f7fffd); -fx-background-radius:22; -fx-padding:22; -fx-border-color:rgba(20,184,166,0.16); -fx-border-radius:22; -fx-effect:dropshadow(gaussian,rgba(15,23,42,0.10),22,0,0,8);"));
    ...
    public void handleBack() {
        try {
            FXMLLoader loader = new FXMLLoader(
                getClass().getResource("/tn/esprit/view/front_CoursModules.fxml"));
        ...
        genSummaryBtn.setOnAction(e -> {
            ...
            Task<String> task = new Task<>() {
                ...
                protected String call() {
                    return SummaryService.getInstance().generateSummary(c.getContenu(), c.getFichierContenu());
                }
            };
    ...
    private void showCoursFormDialog(Cours coursToEdit) {
        ...
        // --- Cloud storage state ---
        final File[] selectedLocalFile = {null};
        final String[] cloudinaryUrl = {null};

        // Initialiser cloudinaryUrl si le cours existant a d\u00e9j\u00e0 une URL Cloudinary
        if (coursToEdit != null && coursToEdit.getFichierContenu() != null
            && CloudinaryService.isCloudinaryUrl(coursToEdit.getFichierContenu())) {
            cloudinaryUrl[0] = coursToEdit.getFichierContenu();
        }

        Button browsePdf = new Button("\ud83d\udcc2 Parcourir");
        browsePdf.setStyle("-fx-background-color:#eff6ff; -fx-text-fill:#2563eb; -fx-background-radius:8; -fx-padding:5 12; -fx-font-size:12; -fx-cursor:hand;");

        // --- Cloud Upload Button ---
        Button uploadCloudBtn = new Button("\u2601 Upload Cloud");
        uploadCloudBtn.setStyle("-fx-background-color:#8b5cf6; -fx-text-fill:white; -fx-font-weight:700; -fx-background-radius:8; -fx-padding:5 12; -fx-font-size:12; -fx-cursor:hand;");
        uploadCloudBtn.setVisible(false);
        uploadCloudBtn.setManaged(false);

        // --- Upload progress ---
        ProgressBar uploadProgressBar = new ProgressBar();
        uploadProgressBar.setPrefWidth(200);
        uploadProgressBar.setVisible(false);
        uploadProgressBar.setManaged(false);
        Label uploadStatusLabel = new Label();
        uploadStatusLabel.setStyle("-fx-text-fill:#64748b; -fx-font-size:11;");
        uploadStatusLabel.setVisible(false);
        uploadStatusLabel.setManaged(false);

        HBox uploadProgressRow = new HBox(8, uploadProgressBar, uploadStatusLabel);
        uploadProgressRow.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        ...
        browsePdf.setOnAction(e -> {
            ...
            if (f != null) {
                if (f.length() > 10 * 1024 * 1024) {
                    ...
                    selectedLocalFile[0] = f;
                    cloudinaryUrl[0] = null;
                    pdfField.setText(f.getAbsolutePath());
                    // Afficher le bouton upload cloud
        ...
        Button okButton = null; // will be set after dialog pane content
        uploadCloudBtn.setOnAction(e -> {
            if (selectedLocalFile[0] == null) {
                new Alert(Alert.AlertType.WARNING, "S\u00e9lectionnez d'abord un fichier PDF.").showAndWait();
                return;
            }
            // Show progress
            uploadProgressBar.setVisible(true); uploadProgressBar.setManaged(true);
            uploadStatusLabel.setVisible(true); uploadStatusLabel.setManaged(true);
            uploadStatusLabel.setText("\u2601 Upload en cours...");
            ...
            Task<String> uploadTask = new Task<>() {
                @Override
                protected String call() {
                    return CloudinaryService.getInstance().uploadPdf(selectedLocalFile[0]);
                }
            };
            ...
            uploadTask.setOnSucceeded(ev -> {
                ...
                if (url != null) {
                    cloudinaryUrl[0] = url;
                    pdfField.setText(url);
                    uploadStatusLabel.setText("\u2705 Upload\u00e9 sur Cloudinary !");
                    uploadProgressBar.setProgress(1.0);
                } else {
                    uploadStatusLabel.setText("\u274c \u00c9chec upload \u2014 le chemin local sera utilis\u00e9.");
                    cloudinaryUrl[0] = null;
                }
            ...
            uploadTask.setOnFailed(ev -> {
                uploadStatusLabel.setText("\u274c Erreur : " + uploadTask.getException().getMessage());
                uploadCloudBtn.setDisable(false);
            });
        ...
        HBox pdfRow = new HBox(8, pdfField, browsePdf, uploadCloudBtn);
        ...
        form.add(uploadProgressRow,           1, 9);
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            // R\u00e9soudre le chemin final du fichier (Cloud > local)
            String pdfPath;
            if (cloudinaryUrl[0] != null) {
                pdfPath = cloudinaryUrl[0];
            } else if (selectedLocalFile[0] != null) {
                pdfPath = selectedLocalFile[0].getAbsolutePath();
            } else {
                String current = pdfField.getText().trim();
                pdfPath = current.isEmpty() ? null : current;
            }
            ...
            if (coursToEdit == null) {
                ...
                // Supprimer l'ancien fichier Cloudinary si remplac\u00e9
                if (pdfPath != null
                    && !pdfPath.equals(coursToEdit.getFichierContenu())
                    && CloudinaryService.isCloudinaryUrl(coursToEdit.getFichierContenu())) {
                    CloudinaryService.getInstance().deletePdf(coursToEdit.getFichierContenu());
                }
                ...
                coursToEdit.setFichierContenu(pdfPath);
    ...
    public void handleBackToModules() {
        try {
            ...
            ctrl.initData(currentUser, currentCategorie);
            Stage stage = (Stage) coursContainer.getScene().getWindow();
            stage.setScene(new Scene(root)); tn.esprit.utils.StageUtils.applyUniformSize(stage); stage.show();
        } catch (IOException e) { e.printStackTrace(); }
    }
    ...
    @FXML public void handleMeet()    { navigateTo("/tn/esprit/view/front_MeetList.fxml"); }
    ...
    private void navigateTo(String fxml) {
        try {
            if ("/tn/esprit/view/front_user_dashboard.fxml".equals(fxml) && currentUser != null) {
                ...
                s.setScene(new Scene(r)); tn.esprit.utils.StageUtils.applyUniformSize(s); s.show(); return;
            }
    ...
    private void handleMoveUp(Cours c) {
        int idx = allCours.indexOf(c);
        if (idx > 0) {
            Cours prev = allCours.get(idx - 1);
            int tmp = c.getOrdre();
            c.setOrdre(prev.getOrdre());
            prev.setOrdre(tmp);
            
            coursController.modifierCours(c);
            coursController.modifierCours(prev);
            loadData(); // Rafraîchir
        }
    }

    private void handleMoveDown(Cours c) {
        int idx = allCours.indexOf(c);
        if (idx != -1 && idx < allCours.size() - 1) {
            Cours next = allCours.get(idx + 1);
            int tmp = c.getOrdre();
            c.setOrdre(next.getOrdre());
            next.setOrdre(tmp);
            
            coursController.modifierCours(c);
            coursController.modifierCours(next);
            loadData(); // Rafraîchir
        }
    }
