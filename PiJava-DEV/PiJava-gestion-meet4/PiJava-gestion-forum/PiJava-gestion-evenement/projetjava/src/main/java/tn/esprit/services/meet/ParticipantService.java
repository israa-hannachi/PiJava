...
public class ParticipantService implements IParticipantsService {
    ...
    @Override
    public List<participant> rechercherParNom(String nom) throws SQLException {
        List<participant> participants = new ArrayList<>();
        String req = "SELECT * FROM participant WHERE nom LIKE ? OR prenom LIKE ?";
        PreparedStatement ps = cnx.prepareStatement(req);
        ps.setString(1, "%" + nom + "%");
        ps.setString(2, "%" + nom + "%");
        ResultSet rs = ps.executeQuery();
    ...
    @Override
    public List<participant> filtrerParRole(String role) throws SQLException {
        List<participant> participants = new ArrayList<>();
        String req = "SELECT * FROM participant WHERE role = ?";
        PreparedStatement ps = cnx.prepareStatement(req);
        ps.setString(1, role);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
    ...
    @Override
    public List<participant> trierParNom(boolean asc) throws SQLException {
        List<participant> participants = new ArrayList<>();
        String order = asc ? "ASC" : "DESC";
        String req = "SELECT * FROM participant ORDER BY nom " + order;
        Statement st = cnx.createStatement();
        ResultSet rs = st.executeQuery(req);
        while (rs.next()) {
            participants.add(mapResultSetToParticipant(rs));
