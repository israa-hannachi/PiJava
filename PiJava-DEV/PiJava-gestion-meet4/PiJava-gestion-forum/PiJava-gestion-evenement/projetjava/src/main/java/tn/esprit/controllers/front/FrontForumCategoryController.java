...
public class FrontForumCategoryController implements Initializable {
    ...
    private void loadCategoryData() {
        ...
        String icone = currentCategory.getIcone();
        String displayIcon = "💬";
        
        if (icone != null && !icone.isEmpty()) {
            String lowerIcone = icone.toLowerCase();
            if (lowerIcone.contains("it.png")) displayIcon = "💻";
            else if (lowerIcone.contains("game.png")) displayIcon = "🎮";
            else if (lowerIcone.contains("dev.png")) displayIcon = "🚀";
            else if (lowerIcone.contains("net.png")) displayIcon = "🌐";
            else if (lowerIcone.contains("shield.png")) displayIcon = "🛡️";
            else if (lowerIcone.contains("edu.png")) displayIcon = "📚";
            else if (lowerIcone.contains("web.png")) displayIcon = "🕸️";
            else displayIcon = icone;
        } else {
            String titre = currentCategory.getTitre().toLowerCase();
            if (titre.contains("info") || titre.contains("it")) displayIcon = "💻";
            else if (titre.contains("game") || titre.contains("jeu")) displayIcon = "🎮";
            else if (titre.contains("dev") || titre.contains("prog")) displayIcon = "🚀";
            else if (titre.contains("réseau") || titre.contains("net")) displayIcon = "🌐";
            else if (titre.contains("cyber") || titre.contains("sécu")) displayIcon = "🛡️";
            else if (titre.contains("stratégie") || titre.contains("entreprise") || titre.contains("edu")) displayIcon = "📚";
            else if (titre.contains("web")) displayIcon = "🕸️";
        }
        
        categoryIcon.setText(displayIcon);
    ...
    private VBox createForumCard(Forum forum, String categoryColor) {
        ...
        String icone = forum.getCategorie() != null ? forum.getCategorie().getIcone() : null;
        String cardIcon = "💬";
        if (icone != null && !icone.isEmpty()) {
            String lowerIcone = icone.toLowerCase();
            if (lowerIcone.contains("it.png")) cardIcon = "💻";
            else if (lowerIcone.contains("game.png")) cardIcon = "🎮";
            else if (lowerIcone.contains("dev.png")) cardIcon = "🚀";
            else if (lowerIcone.contains("net.png")) cardIcon = "🌐";
            else if (lowerIcone.contains("shield.png")) cardIcon = "🛡️";
            else if (lowerIcone.contains("edu.png")) cardIcon = "📚";
            else if (lowerIcone.contains("web.png")) cardIcon = "🕸️";
            else cardIcon = icone;
        }

        Label iconLabel = new Label(cardIcon);
    ...
    private void openForum(Forum forum) {
        // Afficher un curseur d'attente pour donner un retour immédiat
        Scene currentScene = forumsListContainer.getScene();
        if (currentScene != null) {
            currentScene.setCursor(javafx.scene.Cursor.WAIT);
        }

        // Utiliser une petite pause pour laisser l'interface afficher le curseur d'attente
        // avant de geler le thread UI avec le chargement des WebViews (HTMLEditor)
        ...
        pause.setOnFinished(event -> {
            try {
                ...
                if (currentScene != null) {
                    currentScene.setCursor(javafx.scene.Cursor.DEFAULT);
                }
            }
        });
        pause.play();
