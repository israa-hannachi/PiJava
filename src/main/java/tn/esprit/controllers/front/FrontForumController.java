...
import tn.esprit.controllers.ChatbotController;
...
public class FrontForumController implements Initializable {
    ...
    private final String[] categoryIcons = {"💻", "🌐", "🚀", "📊", "🎨", "📚", "🔬", "🎮", "⚙️", "📱", "🛡️", "🕸️", "📈", "🔧", "💡", "📝", "🗂️", "🎯", "🏗️", "🔍", "🎓", "📡", "🖥️", "⚡", "🧪", "📂", "🌟", "🔑", "💼", "📞", "🤖", "🌈", "📸", "💰"};

    @Override
    public void initialize(URL url, ResourceBundle rb) {
        try {
            // UI listener setup only
            setupSearchListener();
    ...
    public void initUser(Users user) {
        ...
        try {
            if (user != null) {
                ...
                if (profile != null && profileImageView != null) {
            ...
            System.err.println("Error in Forum.initUser: " + e.getMessage());
            e.printStackTrace();
            new Alert(Alert.AlertType.ERROR, "Erreur d'initialisation Forum: " + e.toString()).show();
        }
    }
    ...
    private String getIconForCategory(Categorie cat) {
        String icone = cat.getIcone();
        
        // Si l'icône est un placeholder de type .png, on le remplace par un emoji approprié
        if (icone != null && !icone.isEmpty()) {
            String lowerIcone = icone.toLowerCase();
            if (lowerIcone.contains("it.png")) return "💻";
            if (lowerIcone.contains("game.png")) return "🎮";
            if (lowerIcone.contains("dev.png")) return "🚀";
            if (lowerIcone.contains("net.png")) return "🌐";
            if (lowerIcone.contains("shield.png")) return "🛡️";
            if (lowerIcone.contains("edu.png")) return "📚";
            if (lowerIcone.contains("web.png")) return "🕸️";
            
            // Si c'est déjà un emoji ou un autre chemin, on le garde
            return icone;
        }

        // Sinon, on essaie de deviner l'icône à partir du titre
        String titre = cat.getTitre().toLowerCase();
        if (titre.contains("info") || titre.contains("it")) return "💻";
        if (titre.contains("game") || titre.contains("jeu")) return "🎮";
        if (titre.contains("dev") || titre.contains("prog")) return "🚀";
        if (titre.contains("réseau") || titre.contains("net")) return "🌐";
        if (titre.contains("cyber") || titre.contains("sécu")) return "🛡️";
        if (titre.contains("stratégie") || titre.contains("entreprise") || titre.contains("edu")) return "📚";
        if (titre.contains("web")) return "🕸️";

        // Par défaut, utiliser l'ID pour sélectionner une icône
        int iconIndex = Math.abs(cat.getId() - 1) % categoryIcons.length;
    ...
    private HBox createForumCard(Forum forum) {
        ...
        String icon = forum.getCategorie() != null ? getIconForCategory(forum.getCategorie()) : "💬";
        Label iconLabel = new Label(icon);
    ...
    public void openChatbot() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/tn/esprit/view/ChatbotPopup.fxml"));
            Parent root = loader.load();
            
            ChatbotController controller = loader.getController();
            controller.initData(currentUser);
            
            Stage stage = (Stage) floatingChatBtn.getScene().getWindow();
            stage.setMaximized(true);
            stage.setScene(new Scene(root));
            stage.show();
        } catch (IOException e) {
            ...
            e.printStackTrace();
            showChatbotOpenError(e.getMessage());
        }
    }

    private void showChatbotOpenError(String details) {
        Alert alert = new Alert(Alert.AlertType.ERROR);
        alert.setTitle("Erreur");
        alert.setHeaderText("Impossible d'ouvrir l'assistant IA");
        alert.setContentText("Détail: " + details);
        alert.showAndWait();
    }
}
