...
public class BackGameListController {
    ...
    private void handleAddGame() {
        ...
        durationField.setPromptText("Duree en secondes (ex: 60)");
        ...
        grid.add(new Label("Duree (secondes):"), 0, 4);
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == addButtonType) {
                ...
                try {
                    ...
                    newGame.setAttemptNumber(0);
                }
                try {
                    newGame.setDuration(Integer.parseInt(durationField.getText().trim()));
                } catch (NumberFormatException e) {
                    newGame.setDuration(60);
                }
    ...
    private void modifier(Game g) {
        ...
        // Duree — pre-remplie avec la valeur existante
        TextField durationField = new TextField(String.valueOf(g.getDuration()));
        durationField.setPromptText("Duree en secondes (ex: 60)");
        durationField.setPrefWidth(300);
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == saveButtonType) {
                ...
                newGame.setNiveau(niveauField.getText());
                try {
                    newGame.setAttemptNumber(
                            Integer.parseInt(attemptField.getText()));
                } catch (NumberFormatException e) {
                    newGame.setAttemptNumber(0);
                }
                // Duree
                ...
                newGame.setCreatedAt(g.getCreatedAt());
                return newGame;
            }
            return null;
        });
        ...
        dialog.showAndWait().ifPresent(updatedGame -> {
            try {
                gameService.modifier(updatedGame);
                loadGames();
            } catch (Exception e) {
                Alert errorAlert = new Alert(Alert.AlertType.ERROR);
                errorAlert.setTitle("Erreur");
    ...
    private void ajouterQuestion(Game g) {
        ...
        // Si libre : masquer les options visuellement
        if (!"libre".equalsIgnoreCase(g.getType())) {
            grid.add(new Label("Option 1:"), 0, 1);
            grid.add(option1Field, 1, 1);
            grid.add(new Label("Option 2:"), 0, 2);
            grid.add(option2Field, 1, 2);
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == addButtonType) {
                ...
                // ── Controle options : seulement si pas libre ─────────
                if (!"libre".equalsIgnoreCase(g.getType())) {
                    if (option1Field.getText().trim().isEmpty() ||
                            option2Field.getText().trim().isEmpty() ||
                            option3Field.getText().trim().isEmpty() ||
                ...
                // Controle : reponse correcte obligatoire
                if (correctAnswerField.getText().trim().isEmpty()) {
                    Alert alert = new Alert(Alert.AlertType.WARNING);
                    alert.setTitle("Réponse correcte manquante");
                    alert.setHeaderText("Veuillez entrer la réponse correcte !");
                ...
                // ── Controle correspondance : seulement si pas libre ──
                String correct = correctAnswerField.getText().trim();
                if (!"libre".equalsIgnoreCase(g.getType())) {
                    if (!(correct.equals(option1Field.getText().trim()) ||
                            correct.equals(option2Field.getText().trim()) ||
                ...
                newQuestion.setGameId(g.getId());
                newQuestion.setQuestionText(questionTextField.getText().trim());

// Pour libre : option1 = reponse correcte, reste = "-"
                if ("libre".equalsIgnoreCase(g.getType())) {
                    newQuestion.setOption1(correct);
                    newQuestion.setOption2("-");
                    newQuestion.setOption3("-");
                    newQuestion.setOption4("-");
                } else {
        ...
        dialog.showAndWait().ifPresent(newQuestion -> {
            try {
                new GameQuestionController().ajouterQuestion(newQuestion);
                loadQuestions(g.getId());
            } catch (Exception e) {
                Alert errorAlert = new Alert(Alert.AlertType.ERROR);
                errorAlert.setTitle("Erreur");
    ...
    @FXML
    private void handleOpenStats(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/tn/esprit/view/StatsView.fxml")
            );
            Parent view = loader.load();
            BorderPane mainLayout = (BorderPane)
    ...
    @FXML
    private void handleOpenRecommandation(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(
                    getClass().getResource("/tn/esprit/view/RecommandationView.fxml")
            );
            Parent view = loader.load();

            BorderPane mainLayout = (BorderPane)
                    ((Node) event.getSource()).getScene().getRoot();
            mainLayout.setCenter(view); // affiche la recommandation au centre
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void handleBackList(ActionEvent event) {
        navigateSimple("/tn/esprit/view/BackGameList.fxml", event);
    }
