...
import tn.esprit.controllers.forum.CategorieController;
import tn.esprit.controllers.forum.ForumController;
import tn.esprit.controllers.forum.MessageController;
...
public class Main {
    ...
    public static class MainApp extends Application {
        ...
        public void start(Stage primaryStage) {
            try {
                // Vous pouvez changer ici pour "/tn/esprit/view/front_login.fxml" ou "/tn/esprit/view/front_register.fxml"
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/tn/esprit/view/front_user_dashboard.fxml"));
                Parent root = loader.load();
                Scene scene = new Scene(root);
                primaryStage.setTitle("Naja7ni App");
    ...
    public static void main(String[] args) {
        ...
        Application.launch(MainApp.class, args); // Lance l'interface graphique via la classe interne
        ...
        while (choice != 0) {
            ...
            System.out.println("║  6. Gestion des Forums       ║");
    ...
    private static void manageUsers(Scanner scanner) {
        ...
        while (choix != 0) {
            ...
            System.out.println("╠══════════════════════════════════╣");
            System.out.println("║  1. Afficher tous les users      ║");
            System.out.println("║  2. Ajouter un utilisateur       ║");
            System.out.println("║  3. Modifier un utilisateur      ║");
            System.out.println("║  4. Supprimer un utilisateur     ║");
            System.out.println("║  5. Rechercher par email         ║");
            System.out.println("║  6. Filtrer par rôle             ║");
            System.out.println("║  7. Changer statut (ACTIF/BLOQUE)║");
            System.out.println("║  0. Retour au Menu Principal     ║");
            System.out.println("╚══════════════════════════════════╝");
            System.out.print("Votre choix : ");
            if (scanner.hasNextInt()) { choix = scanner.nextInt(); scanner.nextLine(); }
            else { scanner.nextLine(); System.out.println("❌ Choix invalide."); continue; }
            ...
            switch (choix) {
                case 1:
                    ...
                    if (users.isEmpty()) {
                        ...
                        System.out.println("\n------------------------------------------------------------------------");
                        System.out.printf("%-5s %-25s %-15s %-15s %-12s %-8s%n",
                                "ID", "Email", "Prénom", "Nom", "Rôle", "Statut");
                        System.out.println("------------------------------------------------------------------------");
                        for (Users u : users) {
                            System.out.printf("%-5d %-25s %-15s %-15s %-12s %-8s%n",
                                    u.getId(), u.getEmail(), u.getFirstName(),
                                    u.getLastName(), u.getRole(), u.getStatut());
                        }
                    }
                    break;
                ...
                case 2:
                    ...
                    System.out.print("Profession (optionnel) : "); String profession = scanner.nextLine();
                    System.out.print("Niveau d'expérience (optionnel) : "); String expLevel = scanner.nextLine();
                    System.out.print("Rôle (ADMIN / Student / Enseignant) : "); String role = scanner.nextLine();
                    System.out.print("Statut (ACTIF / BLOQUE) [ACTIF] : "); String statut = scanner.nextLine();
                    uc.ajouterUser(new Users(
                            email, pwd, firstName, lastName,
                            profession.isEmpty() ? null : profession,
                            expLevel.isEmpty() ? null : expLevel,
                            role,
                            statut.isEmpty() ? "ACTIF" : statut
                    ));
                    break;
                ...
                case 3:
                    ...
                    int idMod = scanner.nextInt(); scanner.nextLine();
                    Users uMod = uc.findById(idMod);
                    if (uMod == null) {
                        System.out.println("❌ Utilisateur introuvable !");
                    } else {
                        System.out.print("Nouvel email [" + uMod.getEmail() + "] : ");
                        String newEmail = scanner.nextLine();
                        if (!newEmail.isEmpty()) uMod.setEmail(newEmail);
                        System.out.print("Nouveau prénom [" + uMod.getFirstName() + "] : ");
                        String newFirst = scanner.nextLine();
                        if (!newFirst.isEmpty()) uMod.setFirstName(newFirst);
                        System.out.print("Nouveau nom [" + uMod.getLastName() + "] : ");
                        String newLast = scanner.nextLine();
                        if (!newLast.isEmpty()) uMod.setLastName(newLast);
                        System.out.print("Nouvelle profession [" + uMod.getProfession() + "] : ");
                        String newProf = scanner.nextLine();
                        if (!newProf.isEmpty()) uMod.setProfession(newProf);
                        System.out.print("Nouveau rôle [" + uMod.getRole() + "] : ");
                        String newRole = scanner.nextLine();
                        if (!newRole.isEmpty()) uMod.setRole(newRole);
                        System.out.print("Nouveau statut [" + uMod.getStatut() + "] : ");
                        String newStatut = scanner.nextLine();
                        if (!newStatut.isEmpty()) uMod.setStatut(newStatut);
                        uc.modifierUser(uMod);
                    }
                    break;
                ...
                case 4:
                    ...
                    int idSuppr = scanner.nextInt(); scanner.nextLine();
                    uc.supprimerUser(idSuppr);
                    break;

                case 5:
                    System.out.print("Email à rechercher : ");
                    String emailSearch = scanner.nextLine();
                    Users found = uc.findByEmail(emailSearch);
                    if (found == null) System.out.println("❌ Aucun utilisateur trouvé avec cet email.");
                    else System.out.println(found);
                    break;

                case 6:
                    System.out.print("Rôle à filtrer (ADMIN / Student / Enseignant) : ");
                    String roleFilter = scanner.nextLine();
                    List<Users> byRole = uc.findByRole(roleFilter);
                    if (byRole.isEmpty()) {
                        System.out.println("❌ Aucun utilisateur avec ce rôle.");
                    } else {
                        System.out.println("\n--- Utilisateurs avec le rôle : " + roleFilter + " ---");
                        for (Users u : byRole) System.out.println(u);
                    }
                    break;

                case 7:
                    System.out.print("ID de l'utilisateur : ");
                    int idStatut = scanner.nextInt(); scanner.nextLine();
                    System.out.print("Nouveau statut (ACTIF / BLOQUE) : ");
                    String newStatut = scanner.nextLine();
                    uc.changerStatut(idStatut, newStatut);
                    break;
    ...
    // ===================== GESTION DES FORUMS =====================
    private static void manageForums(Scanner scanner) {
        CategorieController catCtrl = new CategorieController();
        ...
        while(choix != 0) {
            System.out.println("\n╔══════════════════════════════╗");
            System.out.println("║       GESTION DES FORUMS     ║");
            System.out.println("╠══════════════════════════════╣");
            System.out.println("║  1. Afficher tous les forums ║");
            System.out.println("║  2. Créer un forum           ║");
            System.out.println("║  3. Supprimer un forum       ║");
            System.out.println("║  4. Gérer les catégories     ║");
            System.out.println("║  5. Gérer les messages       ║");
            System.out.println("║  0. Retour au Menu Principal ║");
            System.out.println("╚══════════════════════════════╝");
            System.out.print("Votre choix : ");
            choix = scanner.nextInt();
            ...
            switch (choix) {
                ...
                case 1:
                    ...
                    for(Forum f : forumCtrl.recupererListeForums()) {
                ...
                case 2:
                    ...
                    System.out.print("Titre de la catégorie existante : ");
                    String titreCat = scanner.nextLine();
                    Categorie catAssociee = catCtrl.trouverCategorie(titreCat);
                    if (catAssociee != null) {
                        forumCtrl.creerForum(titre, desc, auteur, catAssociee);
                        System.out.println("✅ Forum créé avec succès.");
                    } else {
                        System.out.println("❌ Erreur : Catégorie introuvable.");
                    }
                    break;
                case 3:
                    ...
                    scanner.nextLine();
                    forumCtrl.supprimerForum(idSuppr);
                    System.out.println("✅ Forum supprimé.");
                    break;
                case 4:
                    System.out.println("\n--- GESTION DES CATÉGORIES ---");
                    System.out.println("1. Lister | 2. Ajouter | 3. Supprimer");
                    int choixC = scanner.nextInt();
                    scanner.nextLine();
                    if (choixC == 1) {
                        for(Categorie c : catCtrl.listerToutesLesCategories()) {
                        ...
                        catCtrl.ajouterNouvelleCategorie(tc, dc, "default.png");
                    } else if (choixC == 3) {
                        System.out.print("Entrez l'ID de la catégorie à supprimer : ");
                        int idASupprimer = scanner.nextInt();
                        scanner.nextLine();
                        catCtrl.supprimerCategorie(idASupprimer);
                    }
                    break;
                case 5:
                    System.out.println("\n--- GESTION DES MESSAGES ---");
                    System.out.println("1. Voir messages forum | 2. Poster | 3. Stats");
                    int choixM = scanner.nextInt();
                    scanner.nextLine();
                    if (choixM == 1) {
                        System.out.print("ID Forum : ");
                        int idF = scanner.nextInt();
                        scanner.nextLine();

                        for(Message m : msgCtrl.voirMessagesForum(idF)) {
                        ...
                        Forum fDest = new Forum();
                        fDest.setId(idF);
                        msgCtrl.posterMessage(cont, aut, fDest);
                    } else if (choixM == 3) {
                        System.out.print("Nom auteur : ");
                        String aut = scanner.nextLine();
                        PrintStream var10000 = System.out;
                        int var10001 = msgCtrl.statistiquesUtilisateur(aut);
                        var10000.println("Total : " + var10001);
                    }
            }
        }
